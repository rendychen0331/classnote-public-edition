...
internal object WeatherApiImpl {

    private const val TAG = "WeatherApiImpl"
    ...
    private val COUNTY_LOCATIONS: List<WeatherLocation> = listOf(
    ...
    private suspend fun fetchForecastForLocation(
        location: WeatherLocation,
        apiKey: String
    ): Result<List<ForecastItem>> = withContext(Dispatchers.IO) {
        try {
            ...
            Log.e(TAG, "fetchForecast failed for ${location.displayName}", e)
            Result.failure(e)
        }
    }
    ...
    private fun parseCountyForecast(json: String, locationName: String): List<ForecastItem> {
        ...
        val locationsArr = root.getJSONObject("records").getJSONArray("location")
        var targetLocation: JSONObject? = null
        for (i in 0 until locationsArr.length()) {
            val loc = locationsArr.getJSONObject(i)
            if (loc.getString("locationName") == locationName) { targetLocation = loc; break }
        }
    ...
    private fun parseDistrictForecast(json: String, apiName: String): List<ForecastItem> {
        val root = JSONObject(json)
        val locationsArr = root.getJSONObject("records").getJSONArray("Locations")
        var targetLocation: JSONObject? = null
        outer@ for (i in 0 until locationsArr.length()) {
            ...
            for (j in 0 until locationArr.length()) {
                ...
                if (loc.optString("LocationName") == apiName) { targetLocation = loc; break@outer }
            }
        }
        targetLocation ?: return emptyList()
