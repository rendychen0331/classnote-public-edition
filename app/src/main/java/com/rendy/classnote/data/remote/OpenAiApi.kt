...
object OpenAiApi {
    ...
    suspend fun analyzeNotifications(
        ...
        ApiLogger.log("openai(通知辨識)", prompt.take(300), responseText?.take(300), duration, success)
    ...
    suspend fun chatWithContext(
        ...
        val duration = measureTimeMillis {
            try {
                ...
                if (code != 200) {
                    ...
                    ApiLogger.log("openai(筆記對話)", userMessage.take(100), "HTTP $code: $err", 0, false)
                    return@measureTimeMillis
                }
        ...
        ApiLogger.log("openai(筆記對話)", userMessage.take(100), result?.take(200), duration, result != null)
        result
    }
}
